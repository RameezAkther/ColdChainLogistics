...
MQTT_TOPIC = [("coldchain/truck1/sensor2", 0), ("coldchain/truck1/sensor2/location", 0)]
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        sensor_id = topic_parts[2] if len(topic_parts) >= 3 else payload.get("sensor_id", "sensor2")
        payload["sensor_id"] = sensor_id
        payload["timestamp"] = datetime.utcnow()

        # --- Topic: coldchain/truck1/sensor2 (Sensor Data) ---
        if msg.topic == "coldchain/truck1/sensor2":
            ...
            PRESSURE_THRESHOLD = float(thresholds.get("bmp280", {}).get("pressure", 1500))
            ALTITUDE_THRESHOLD = float(thresholds.get("bmp280", {}).get("altitude", 200))
            # print(TEMP_THRESHOLD)
            # print(HUMIDITY_THRESHOLD)
            alerts = []
            if float(payload["temperature"]) > TEMP_THRESHOLD:
                alerts.append(f"High Temperature: {payload['temperature']}°C")
            if float(payload["humidity"]) > HUMIDITY_THRESHOLD:
                alerts.append(f"High Humidity: {payload['humidity']}%")
            if float(payload["airQuality"]) > GAS_THRESHOLD:
                alerts.append(f"Poor Air Quality: {payload['airQuality']} PPM")
            if float(payload["pressure"]) > PRESSURE_THRESHOLD:
                alerts.append(f"Pressure: {payload['pressure']} hPa")
            if float(payload["altitude"]) > ALTITUDE_THRESHOLD:
                alerts.append(f"Altitude: {payload['altitude']} m")
        ...
        # --- Topic: coldchain/truck1/sensor2/location (GPS + WiFi) ---
        elif msg.topic == "coldchain/truck1/sensor2/location":
